...
public class UnitPathAssignmentFlowGraph
{
    ...
    private static void assigner(List<UnitPathAssignmentFlowGraph> pathsForUnits, HashSet<Unit> units, List<List<Vector2>> gridPaths)
    {
        ...
        foreach (var l in level)
        {
            if (l.paths.Count == 1)
            {
                AssignPathToUnits(l.paths[0], l.assignedUnits, gridPaths);
                units.ExceptWith(l.assignedUnits);
                continue;
            }

            assigner(l.paths, l.assignedUnits, gridPaths);
        }
    }

    private static void AssignPathToUnits(UnitPathAssignmentFlowGraph unitPathAssignment, HashSet<Unit> assignedUnits, List<List<Vector2>> gridPaths)
    {
    ...
    internal static void AssignUnitPathsHeuristic(List<UnitPathAssignmentFlowGraph> pathsForUnits)
    {
        //AssignPathsToUnitsHeuristicallyByPathCloseness(pathsForUnits);
        //AssignPathsToUnits(pathsForUnits);
        AssignPathsToUnitsHeuristically(pathsForUnits);
    }
    ...
    private static void AssignPathsToUnitsHeuristically(List<UnitPathAssignmentFlowGraph> pathsForUnits)
    {
        ...
        assigner(pathsForUnits, units, gridPaths);
    }
